...
model_bas_ske_pb <- 
  brm(
    ...
    cores = 2,
    chains = 2,
    file = "Model_base_skeptic_prior",
    control = list(adapt_delta = 0.99, max_treedepth = 20))

```

```{r prior check - not updated}
#pp checking the priors - could look better, but fine i guess
#pp <- 
  pp_check(model_all_inf_pa, ndraws = 100)+labs(title='Train Informed')
...
```{r posterior check - not updated}
#pop <- 
  pp_check(fit_inf_all, ndraws = 100)+labs(title='Train Informed')+xlim(-1,3)
...
##combine
pop_inf_all <- grid.arrange(variables, sd,intercept, top='Prior-posterior Informed',ncol=2)
ggsave("prior_posterior_inf_all.PNG",pop_inf_all,height=7,width=10,units="in")
...
##combine
pop_inf_int <- grid.arrange(variables, sd, intercept, top='Prior-posterior Informed - Varying intercepts',ncol=2)
ggsave("prior_posterior_update_inf_int.PNG",pop_inf_int,height=7,width=10,units="in")
...
##combine
pop_ske_all <- grid.arrange(variables, sd, intercept, top='Prior-posterior Skeptic - varying slopes and intercept',ncol=2)
...
##combine
pop_ske_int <- grid.arrange(variables, sd, intercept, top='Prior-posterior Skeptic - Varying intercepts',ncol=2)
ggsave("prior_posterior_update_ske_int.PNG",pop_ske_int,height=7,width=10,units="in")
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(200, 200)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(191,9,11,189)))
#create confusion matrix and calculate metrics related to confusion matrix
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(800, 800)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(795,5,7,793)))
#create confusion matrix and calculate metrics related to confusion matrix
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(800, 800)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(452,348,319,481)))
#create confusion matrix and calculate metrics related to confusion matrix
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(800, 800)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(458,342,325,475)))
#create confusion matrix and calculate metrics related to confusion matrix
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(200, 200)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(133,67,70,130)))
#create confusion matrix and calculate metrics related to confusion matrix
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(200, 200)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(128,72,80,120)))
#create confusion matrix and calculate metrics related to confusion matrix
...
#define vectors of actual values and predicted values
actual <- factor(rep(c('SC', 'HC'), times=c(200, 200)))
pred <- factor(rep(c('SC', 'HC', 'SC', 'HC'), times=c(132,68,77,123)))
#create confusion matrix and calculate metrics related to confusion matrix
...
sum(unique(test_ff$NewID)==unique(train_f$NewID))
...
```{r Bayesian}
#make model
em <- bf(Diagnosis ~ MR2 + MR5 + MR9 + MR6 + MR3 + MR4 + MR1 + MR7 + MR8 +(1|NewID))
...
pop_em <- grid.arrange(variables,sd_int, top='Prior-posterior Empirical',nrow=2)
pop_em
...
train <- fa_tf %>% 
 ...
 train$PredictionsPerc <- predict(fit_em)[, 1]
 train$Predictions[train$PredictionsPerc > 0.5] <- "SCZ"
 train$Predictions[train$PredictionsPerc <= 0.5] <- "CT"
...
test$PredictionsPerc <- predict(fit_em, newdata=fa_tef, allow_new_levels=TRUE)[, 1]
test$Predictions[test$PredictionsPerc > 0.5] <- "SCZ"
test$Predictions[test$PredictionsPerc <= 0.5] <- "CT"
...
# )
# #define vectors of actual values and predicted values
# actual <- factor(rep(c('SCZ', 'CT'), times=c(900, 989)))
# pred <- factor(rep(c('CT', 'SCZ', 'CT', 'SCZ'), times=c(989,0,0,900)))
# #create confusion matrix and calculate metrics related to confusion matrix
# #library(caret)
# conf_mat_test_em <- confusionMatrix(pred, actual, mode = "everything", positive="SC")

Per_em <- tibble(expand_grid(
  Sample=seq(2000), # remember this is from brm(iter=XX), sample=seq(XX)
  Model=c('VaryingIntercepts'), #add more coloumns, if you have more models
  Type = c('Train', 'Test'),
  Accuracy = NULL
))

train_inv_em <- inv_logit_scaled(posterior_linpred(fit_em,summary=F))
test_inv_em <- inv_logit_scaled(posterior_linpred(fit_em, summary=F, newdata=fa_tef, allow_new_levels=T))

Per_em <- Per_em %>% 
  mutate(Accuracy=NA)

for (i in seq(2000)){
    train$Predictions <- as.factor(ifelse(train_inv_em[i,]>0.5, "SCZ","CT"))
    test$Predictions <- as.factor(ifelse(test_inv_em[i,]>0.5, "SCZ","CT"))
    
    Per_em$Accuracy[Per_em$Sample == i & Per_em$Model == "VaryingIntercepts"  & Per_em$Type == "Train"] <- accuracy(train, truth=Diagnosis, estimate=Predictions)[,'.estimate']
    
    Per_em$Accuracy[Per_em$Sample == i & Per_em$Model == "VaryingIntercepts"  & Per_em$Type == "Test"] <- accuracy(test, truth=Diagnosis, estimate = Predictions)[,'.estimate']
}
